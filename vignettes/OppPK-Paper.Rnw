...
The two-compartment model is commonly used to model pharmacokinetic processes for intraevenously infused drugs. It is expressed as a system of two ordinary differential equations as follows, where $m_1$ and $m_2$ are the masses of drug in the central and peripheral compartments, respectively.
