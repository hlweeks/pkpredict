...
By default, the \texttt{mic_stat} function uses the Laplace approximation to calculate an approximate credible interval for target attainment. The user can also specify the argument \texttt{mcmc = TRUE} to use MCMC to obtain a more precise credible interval.

<<>>=
# Posterior estimate after observed measurements
post_ft.mic_mcmc <- mic_stat(ivt = ivt_d, dat = dat_d, th = 64, mcmc = TRUE)
post_ft.mic_mcmc
@
...
% write print method, other necessary commentary on the model object
...
Below we extract the predicted values with the \texttt{predict.pkm} method. By default, the \texttt{predict} method returns the estimated concentration of drug in the bloodstream at the supplied start and end of infusion times, and (if applicable) the times associated with concentration measurements from the model. Alternatively, the user can supply specific times at which to compute the estimated drug concentration.
