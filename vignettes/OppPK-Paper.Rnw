...
ggplot(ncov_long, aes(x = type, y = ncov, fill = type)) +
  facet_grid(~ side) + geom_violin(na.rm = TRUE) +
  scale_fill_manual(values = cbColors)
...
We see that the MIC statistic is exactly that produced by the \texttt{mic_stat} function. Below we extract the predicted values, either directly from the model object (returns values calculated at infusion and blood draw times), or at user-specified times with the \texttt{predict.pkm} method.
