...
The two-compartment model is commonly used to model pharmacokinetic processes for intraevenously infused drugs. We focus on the intravenous infusion version of the two-compartment model because our application calls for it. It is expressed as a system of two ordinary differential equations as follows, where $m_1$ and $m_2$ are the masses of drug in the central and peripheral compartments, respectively.
...
We considered several possible functional forms of $h(\log \theta_i)$ for \mic:

$$\begin{aligned}
\text{Linear: }& h(log \theta_i) = log\ \theta_i \\
\text{Logit-transformed: }& h(log \theta_i) = log\ \cfrac{\theta_i}{1 - \theta_i} \\
\text{Probit-transformed: }& h(log \theta_i) = \Phi^{-1}(log\ \theta_i) \\
\end{aligned}$$
