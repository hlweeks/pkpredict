...
% [MSS note: I think this text belongs in the methods] Dosing for a patient needs to be enough that the concentration in the blood remains above the necessary threshold for suppression of the offending microorganism (usually a multiple of the minimum inhibitory concentration), yet not so high as to increase the risk of adverse side effects.\\
...
% check if uniroot is a bisection method of finding the root - if not bisection, then identify what method it's using
