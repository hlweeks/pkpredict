...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in library(pkpredict): there is no package called 'pkpredict'}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}ivt\_toList"{}}}\end{kframe}
\end{knitrout}

% latex table generated in R 3.3.3 by xtable 1.8-2 package
% Thu May 18 13:47:32 2017
\begin{table}[ht]
\centering
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}mic\_stat"{}}}\begin{alltt}
\hlstd{prior_ft.mic}\hlopt{$}\hlstd{ftmic}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'prior\_ft.mic' not found}}\begin{alltt}
\hlcom{# Default parameters used}
\hlstd{(pars} \hlkwb{<-} \hlstd{prior_ft.mic}\hlopt{$}\hlstd{pars)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'prior\_ft.mic' not found}}\begin{alltt}
\hlcom{# Posterior estimate after observed measurements}
\hlstd{est} \hlkwb{<-} \hlkwd{optim}\hlstd{(pars, log_posterior,} \hlkwc{ivt} \hlstd{= ivt_d,} \hlkwc{dat} \hlstd{= dat_d,}
             \hlkwc{control} \hlstd{=} \hlkwd{list}\hlstd{(}\hlkwc{fnscale}\hlstd{=}\hlopt{-}\hlnum{1}\hlstd{),} \hlkwc{hessian}\hlstd{=}\hlnum{TRUE}\hlstd{)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in optim(pars, log\_posterior, ivt = ivt\_d, dat = dat\_d, control = list(fnscale = -1), : object 'pars' not found}}\begin{alltt}
\hlstd{post_ft.mic} \hlkwb{<-} \hlkwd{mic_stat}\hlstd{(}\hlkwc{ivt} \hlstd{= ivt_d,} \hlkwc{th} \hlstd{=} \hlnum{64}\hlstd{,} \hlkwc{pars} \hlstd{= est}\hlopt{$}\hlstd{par)}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}mic\_stat"{}}}\begin{alltt}
\hlstd{post_ft.mic}\hlopt{$}\hlstd{ftmic}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'post\_ft.mic' not found}}\end{kframe}
\end{knitrout}

In order to get confidence intervals for these estimates, it is necessary to fit the PK model.
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): could not find function "{}pkm"{}}}\begin{alltt}
\hlstd{pk.fit}\hlopt{$}\hlstd{ftmic}
\end{alltt}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'pk.fit' not found}}\end{kframe}
\end{knitrout}

\begin{kframe}


{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'pk.fit' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in names(pred.df) <- c("{}Time Since First Infusion (h)"{}, "{}Concentration (mcg/mL)"{}, : object 'pred.df' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in xtable(pred.df, digits = 2, caption = "{}Predicted Values"{}): object 'pred.df' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in align(xdf) <- rep("{}c"{}, 4): object 'xdf' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in print(xdf, caption.placement = "{}top"{}, include.rownames = FALSE): object 'xdf' not found}}\end{kframe}
