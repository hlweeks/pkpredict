...
\end{alltt}
\begin{verbatim}
##   lv_1  lk_10  lk_12  lk_21   lerr 
##  3.223 -1.650 -7.000 -7.000  2.330
\end{verbatim}
\end{kframe}
\end{knitrout}

Out prior estimate for target attainment indicates that on average, a patient on the specified infusion schedule will
({\bf does the prior indicate the response of an average patient or average response across a variety of patients?})
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in pkm(formula = con \textasciitilde{} time, data = dat\_d, ivt = ivt\_d, thres = 64): object 'xx' not found}}\begin{alltt}
